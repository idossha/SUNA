...
├── python/
│   ├── suna_mpl/           matplotlib companion (a pip package; NOT shipped — §16.1)
...
the app only runs those scripts. **It is not staged into the packaged app and is not published**
— see §20.6.
...
* **What goes inside the bundle: `docs/packaging.md`.** `electron-builder`, configured by
  ...
  `node_modules` for its two external dependencies, and `python/suna_kernel`.
...
that is three resolvers — the example project, `mcp/server.mjs`, and `python/suna_kernel/bridge.py`.
...
**20.6 `suna_mpl` is scaffolded but not delivered.** `starter-scaffold.ts` writes
`import suna_mpl` into the starter `plot.py`, but `suna_mpl` is not staged into the bundle and is
not on PyPI. A packaged-app user who runs the scaffolded script gets `ModuleNotFoundError` unless
they install it from a source checkout. The same shape recurs with notebooks: the kernel bridge
enumerates `no-jupyter-client` with a `pip install ipykernel` remedy, but nothing in onboarding
installs it, so a user who completes the wizard and opens a notebook hits that error on first
run.
