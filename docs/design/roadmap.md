...
| Journal profiles | **9 journal profiles** (plus the SUNA house style) from official author guidelines with per-value source URLs + provenance tags (`documented` / `counted-empirically` / `inferred`); figure & manuscript compliance checkers; the canvas export presets are driven by the *active* profile (`Double column (183 mm)` for Nature, not a hardcoded width). feature-plan-6 §1 added the neuroscience set — Nature, Neuron, PNAS, Brain Stimulation, SLEEP, Sleep Advances, J. Neural Engineering, J. Neuroscience — beside Science; the three astronomy profiles that shipped first (Nature Astronomy / ApJ / MNRAS) were later removed. **Anything a journal does not state is `null`, never guessed**; where a publisher's site refused every fetch (Elsevier/Cell/PNAS/SfN all returned HTTP 403), the affected fields stay null and the profile's `notes[]` says so rather than borrowing a sibling journal's rules |
...
| Study acquisition | adr-007 / feature-plan-10: a free-text mention → a PDF-backed citation, with the agent always naming which of four outcomes happened. `find_study` asks all four keyless providers **in parallel**, merges on DOI → arXiv id → folded title and ranks (a DOI/arXiv hint is decisive; `citedByCount` breaks ties only), reporting confidence, up to 4 alternatives with their DOIs, and every provider that failed **by name** — a metered OpenAlex 429 is never "no such paper". `find_local_pdf` searches this machine read-only: Spotlight (`mdfind` through `execFile` with an argv array — no shell) plus a bounded walk of the roots in `~/SunaConfig/library.json`, scoring filenames first and then reading the first 256 KB of 12 candidates for the DOI / arXiv id / title in the bytes (uncompressed XMP makes that work without a PDF parser); missing roots are dropped and reported, the file cap sets `truncated`. That budget of 12 goes to the filename-ranked candidates **first and then to the ones whose names scored nothing at all** — Zotero, one of the four default roots, files everything as `storage/<8 chars>/Full Text PDF.pdf`, a name no filename rule matches, so reading only the top 12 of the ranked list left `doi-in-bytes`, the strongest evidence there is, unreachable for exactly the layout that most needs it. `fetch_pdf` and `cite_study` run the ladder — `already-present` → `copied-local` (**copy, never move**) → `downloaded` (**mirrors before publishers** — arXiv, bioRxiv/medRxiv, then *every* OA location OpenAlex lists for the DOI with repositories ranked ahead of publishers and PMC ids fetched through europepmc.org, and only then the record's own `openAccessUrl`, Unpaywall, and the publisher page's `citation_pdf_url` under the `publisher` policy. Measured 2026-08-18 over ten specimens: reading only `best_oa_location` downloaded 4 of 10, walking every location 8 of 10 — that field names the *publisher*, which is the host most likely to refuse a script, while the arXiv and PubMed Central copies beside it serve bytes. A failure now says **why** — `refused` (401/403 or an interstitial, with the hosts named, so the user can open a free-but-Cloudflared paper in a browser) vs `no-open-copy` vs `unreachable`, classified from the HTTP status and rendered by one shared `describePdfFailure`; byte-verified, 50 MB and 3-redirect caps under **one 60 s ceiling for the whole `downloadPdf` call** — the 20 s hop timeout is a sub-limit of that shared deadline, not a per-URL cap that multiplies by rungs and redirect hops into minutes of a wedged IPC call; **no Sci-Hub or proxy fallback** — a 403 is reported as a 403) → `metadata-only`. Reads may leave the project, only into the user's configured roots; **writes never do** — `resolveInside` is a lexical check that a symlinked `references/` walks straight through, so what actually holds the boundary is `prepareReferencesDir` realpath-resolving **both** the directory and the project root and re-asserting the prefix before any bytes are written. A local file is copied unasked only when the evidence names the *work*: `high`, or `medium` corroborated by a second distinct evidence id (`isAutoCopyable`). A lone `filename-author-year` is an uncorroborated `medium` and is **not** copied — "Smith 2020" names every paper Smith wrote in 2020 — it is reported as a candidate with its path and evidence, copied only when someone accepts it by name (`fetch_pdf {"accept": "<that path>"}`, and only a path the scan itself reported). `cite_study` re-reads `references.bib` fresh after the download so a slow fetch cannot clobber the app's concurrent write. **Low confidence writes nothing at all** — no bib entry, no PDF — and hands back the candidates with their DOIs, because guessing which paper the user meant is the one failure this feature must not have. Surfaced three ways from one implementation: the four MCP verbs, Settings → *Reference library* (roots, Spotlight toggle, download policy), and *Find PDF* on a References row. **Unit-tested end to end** with injected providers and a real temp-tree scan (no network, no Spotlight); the download ladder and the live scan have **no smoke step** and have not been driven in the real app under automation |
