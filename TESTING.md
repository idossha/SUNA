...
`code/stripping_model.py`), and the Nature profile active in
`suna.json`. (A project you create yourself starts in **SUNA style**
instead — see "SUNA style" below; the example keeps a journal profile so the
journal-specific behaviour has something to exercise.)
