 ...
 * that renders as "**Figure N.** *title*" under an embed and as the caption /
 * "Note." lines around a `![[tbl:id]]` table.
 *
 * Figure captions live in `figures/<id>/figure.json` (`caption.title`); table
...
const figureCache = new Map<string, CacheEntry<string | null>>()
...
/** The caption title from `figures/<id>/figure.json`, or null when unreadable. */
export async function loadFigureCaptionTitle(rootDir: string, figureId: string): Promise<string | null> {
  ...
  const caption = (parsed as { caption?: { title?: unknown } } | null)?.caption
  const value = typeof caption?.title === 'string' ? caption.title : null
...
/**
 * Write a new caption title into `figures/<id>/figure.json`. Read-fresh →
 * patch one field → write, so every other field survives untouched. Returns
 * false (and writes nothing) when the file cannot be read or parsed.
 */
export async function saveFigureCaptionTitle(
  rootDir: string,
  figureId: string,
  title: string
): Promise<boolean> {
