...
Trailing sentence punctuation is dropped from a narrative key, and things that merely contain an `@` — `word@key2020`, `ihaber@wisc.edu` — are not treated as citations. `@fig:`, `@tbl:`, `@eq:` and `@sec:` are [cross-references](/writing/scimark), not citations; an unrecognised prefix such as `@data:release` *is* parsed as a citation with the key `data:release`.
