...
Twenty-three verbs, all returning plain text. A verb that fails comes back as text flagged `isError`, not as a protocol error, so an agent can read the failure and retry.
